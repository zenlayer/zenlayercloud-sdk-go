...
func NewDescribeImageCopyProgressRequest() (request *DescribeImageCopyProgressRequest) {
	request = &DescribeImageCopyProgressRequest{
		BaseRequest: &common.BaseRequest{},
	}
	request.Init().InitWithApiInfo(SERVICE, APIVersion, "DescribeImageCopyProgress")

	return
}

func NewDescribeImageCopyProgressResponse() (response *DescribeImageCopyProgressResponse) {
	response = &DescribeImageCopyProgressResponse{
		BaseResponse: &common.BaseResponse{},
	}
	return
}

// DescribeImageCopyProgress 查询自定义镜像在各个目标区域的复制进度。
func (c *Client) DescribeImageCopyProgress(request *DescribeImageCopyProgressRequest) (response *DescribeImageCopyProgressResponse, err error) {
	response = NewDescribeImageCopyProgressResponse()
	err = c.ApiCall(request, response)
	return
}
...
func NewDescribeInterconnectBorderGatewayRegionsRequest() (request *DescribeInterconnectBorderGatewayRegionsRequest) {
	request = &DescribeInterconnectBorderGatewayRegionsRequest{
		BaseRequest: &common.BaseRequest{},
	}
	request.Init().InitWithApiInfo(SERVICE, APIVersion, "DescribeInterconnectBorderGatewayRegions")

	return
}

func NewDescribeInterconnectBorderGatewayRegionsResponse() (response *DescribeInterconnectBorderGatewayRegionsResponse) {
	response = &DescribeInterconnectBorderGatewayRegionsResponse{
		BaseResponse: &common.BaseResponse{},
	}
	return
}

// DescribeInterconnectBorderGatewayRegions 查询当前账号下边界网关可支持互联的节点与数据中心信息。边界网关可绑定 CloudRouter 以实现 VPC 的 L3 互联，本接口返回支持此功能的节点列表，可按节点ID或数据中心进行过滤。
func (c *Client) DescribeInterconnectBorderGatewayRegions(request *DescribeInterconnectBorderGatewayRegionsRequest) (response *DescribeInterconnectBorderGatewayRegionsResponse, err error) {
	response = NewDescribeInterconnectBorderGatewayRegionsResponse()
	err = c.ApiCall(request, response)
	return
}
...
func NewCreateQosPolicyGroupRequest() (request *CreateQosPolicyGroupRequest) {
	request = &CreateQosPolicyGroupRequest{
		BaseRequest: &common.BaseRequest{},
	}
	request.Init().InitWithApiInfo(SERVICE, APIVersion, "CreateQosPolicyGroup")

	return
}

func NewCreateQosPolicyGroupResponse() (response *CreateQosPolicyGroupResponse) {
	response = &CreateQosPolicyGroupResponse{
		BaseResponse: &common.BaseResponse{},
	}
	return
}

// CreateQosPolicyGroup 创建一个QoS策略组，将EIP、IPv6或UNMANAGED出口IP纳入同一策略组实施共享带宽限速。
func (c *Client) CreateQosPolicyGroup(request *CreateQosPolicyGroupRequest) (response *CreateQosPolicyGroupResponse, err error) {
	response = NewCreateQosPolicyGroupResponse()
	err = c.ApiCall(request, response)
	return
}
...
// ModifySubnetStackType 修改子网的IP堆栈类型，用于分配或移除子网上的IPv6。
...
// CreatePlacementGroup 创建一个置放组。置放组的管理范围为zone维度，创建时必须选择所属可用区。
...
// ModifyPlacementGroupAttributes 修改置放组的名称、分区数、亲和度等属性。所属可用区不可修改。分区数只能调大，不能调小。
...
// ModifyHaVipAttribute 修改高可用虚拟IP的名称或绑定的安全组。name 与 securityGroupId 至少提供一个。
